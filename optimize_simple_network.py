...
@click.option("--config-file-path", type=click.Path(exists=True, file_okay=True, dir_okay=False),
              default='config/optimize_simple_network_J_config.yaml')
        ...
        if export:
            ...
            with h5py.File(context.temp_output_path, 'a') as f:
                ...
                if shared_context_key not in group:
                    ...
                    subgroup.create_dataset('binned_t_edges', data=binned_t_edges, compression='gzip')
                    subgroup.create_dataset('full_rec_t', data=full_rec_t, compression='gzip')
...
def analyze_network_output_replay(network, ensemble_id=None, trial=None, model_id=None, export=False, plot=False):
    ...
    if context.debug and context.verbose > 0 and context.comm.rank == 0:
        print(
            'optimize_simple_network: analyze_network_output_replay: pid: %i; model: %s; trial: %i; gathering data '
            'across ranks took %.2f s' % (os.getpid(), str(model_id), trial, time.time() - start_time))
        sys.stdout.flush()
        current_time = time.time()
    ...
    if context.comm.rank == 0:
        ...
        if context.debug and context.verbose > 0:
            print(
                'optimize_simple_network: analyze_network_output_replay: pid: %i; model: %s; trial: %i; merging data '
                'structures for analysis took %.2f s' % (os.getpid(), str(model_id), trial, time.time() - current_time))
            current_time = time.time()
            sys.stdout.flush()
        ...
        if context.debug and context.verbose > 0:
            print(
                'optimize_simple_network: analyze_network_output_replay: pid: %i; model: %s; trial: %i; additional '
                'data analysis took %.2f s' % (os.getpid(), str(model_id), trial, time.time() - current_time))
            sys.stdout.flush()
        ...
        if export:
            ...
            with h5py.File(context.temp_output_path, 'a') as f:
                ...
                if shared_context_key not in group:
                    ...
                    set_h5py_attr(subgroup.attrs, 'network_id', context.network_id)
                    set_h5py_attr(subgroup.attrs, 'network_instance', context.network_instance)
                    set_h5py_attr(subgroup.attrs, 'trial_offset', context.replay_trial_offset)
                    set_h5py_attr(subgroup.attrs, 'connectivity_type', context.connectivity_type)
                    set_h5py_attr(subgroup.attrs, 'duration', context.duration)
                    set_h5py_attr(subgroup.attrs, 'buffer', context.buffer)
                    set_h5py_attr(subgroup.attrs, 'stim_epochs', str(context.replay_stim_epochs))
                    set_h5py_attr(subgroup.attrs, 'active_rate_threshold', context.active_rate_threshold)
                    data_group = subgroup.create_group('pop_gid_ranges')
                    subgroup.create_dataset('full_rec_t', data=full_rec_t, compression='gzip')
                    for pop_name in context.pop_gid_ranges:
