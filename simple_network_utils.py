...
def get_run_trial_data_from_file(run_data_file_path, trial_key, baks_alpha, baks_beta, baks_pad_dur, baks_wrap_around):
    """
    ...
    :param baks_pad_dur: float
    :param baks_wrap_around: bool
    :return: tuple (array, dict, dict)
    """
    start_time = time.time()
    if not os.path.isfile(run_data_file_path):
        raise IOError('get_run_trial_data_from_file: invalid run_data_file_path: %s' % run_data_file_path)
    ...
    print('debug: pid: %i took %.1f s to load spikes times for trial: %s' % (os.getpid(), time.time() - start_time,
                                                                             trial_key))
    sys.stdout.flush()
    ...
    for pop_name in binned_firing_rates_dict:
        if pop_name in tuning_peak_locs:
            ...
            this_target_gids = [int(gid_key) for gid_key in binned_firing_rates_dict[pop_name]]
            sorted_gid_dict[pop_name] = np.array(sorted(this_target_gids), dtype='int')

    firing_rates_matrix_dict = dict()
    for pop_name in binned_firing_rates_dict:
        firing_rates_matrix_dict[pop_name] = np.empty((len(binned_firing_rates_dict[pop_name]), len(binned_t)))
        for i, gid in enumerate(sorted_gid_dict[pop_name]):
            firing_rates_matrix_dict[pop_name][i, :] = binned_firing_rates_dict[pop_name][gid]

    print('debug: pid: %i took %.1f s to process firing rates for trial: %s' %
          (os.getpid(), time.time() - current_time, trial_key))
    sys.stdout.flush()

    return binned_t, firing_rates_matrix_dict, sorted_gid_dict
