...
export MODEL_KEY="$6"
export LABEL=simple_network_replay_"$MODEL_LABEL"_"$NETWORK_ID"
...
do
  ...
  ibrun -n 1120 python3 simulate_simple_network_replay.py --config-file-path=$CONFIG_FILE_PATH --verbose=1 \
      ...
      --model_key=$MODEL_KEY --network_id=$NETWORK_ID --network_instance="\$NETWORK_INSTANCE" \
      --label="$LABEL"_"\$NETWORK_INSTANCE --merge-output-files --output-dir=$SCRATCH/data/optimize_simple_network
done
EOT
